...
// app/api/transactions/actions/create-transaction.ts
'use server';
...
export async function createTransaction(formData: FormData) {
  ...
  try {
    ...
    // Validate accountId and categoryId belong to user
    const [accountCheck] = await conn.execute<RowDataPacket[]>(
    ...
    const [categoryCheck] = await conn.execute<RowDataPacket[]>(
    ...
    await conn.execute(
      ...
      [id, userId, new Date(date), accountId, categoryId, payee, Number(amount), notes || null, new Date()]
    );
